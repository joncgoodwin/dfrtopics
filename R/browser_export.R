...
#' Output data files for dfr-browser (hathitrust version)
#'
#' Transform and save modeling results in a format suitable for use by
#' \href{http://agoldst.github.io/dfr-browser}{dfr-browser}, the web-browser
#' based model browser.
...
#'
#' \dontrun{
#' m <- model_hathi_documents("citations.CSV", "wordcounts",
#'     "stoplist.txt", n_topics=40)
#' export_ht_browser_data(m, out_dir="data")
#' }
#'
#' @seealso \code{\link{model_hathi_documents}}, \code{\link{train_model}},
#'   \code{\link{topic_scaled_2d}}
#'
#' @export
export_ht_browser_data <- function (m, out_dir, zipped=TRUE,
                                 n_top_words=50,
                                 download_dfb=FALSE,
                                 overwrite=FALSE) {
    if (!requireNamespace("jsonlite", quietly=TRUE)) {
